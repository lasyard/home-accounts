...
void Sqlite3Store::open()
{
    ...
    std::string content;
    readSection(CHECK_STUB, content);
}
...
void Sqlite3Store::forEachSection(std::function<bool(const std::string &)> callback) const
{
    ...
    while (rc != SQLITE_DONE) {
        if (rc == SQLITE_ROW) {
            ...
            if (!callback(std::string(name, size))) {
                break;
            }
...
const byte *Sqlite3Store::readRawSection(
...
void Sqlite3Store::writeRawSection(const std::string &name, const std::string &content, const byte key[CRYPTO_KEY_LEN])
{
    prepareSql(
        m_putStmt,
    ...
    sqlite3_bind_blob(m_putStmt, 3, content.c_str(), static_cast<int>(content.size()), SQLITE_STATIC);
...
void Sqlite3Store::init()
{
    ...
    writeSection(CHECK_STUB, "The journey of a thousand miles begins with one step.");
}
